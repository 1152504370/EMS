...
(function(undefined) {
	...
	var moment, VERSION = "2.5.1", global = this, round = Math.round, i,
	...
	isoTimes = [ [ 'HH:mm:ss.SSSS', /(T| )\d\d:\d\d:\d\d\.\d{1,3}/ ],
	...
	function dateFromConfig(config) {
		...
		if (config._w && config._a[DATE] == null && config._a[MONTH] == null) {
			fixYear = function(val) {
				...
				return val ? (val.length < 3 ? (int_val > 68 ? 1900 + int_val
						: 2000 + int_val) : int_val)
						: (config._a[YEAR] == null ? moment().weekYear()
								: config._a[YEAR]);
			};
					...
					if (input != null) {
						...
						if (offset !== input) {
							addOrSubtractDurationFromMoment(this, moment
									.duration(offset - input, 'm'), 1, true);
						}
					} else {
	...
	for (i = 0; i < proxyGettersAndSetters.length; i++) {
		makeGetterAndSetter(proxyGettersAndSetters[i].toLowerCase().replace(
				/s$/, ''), proxyGettersAndSetters[i]);
	}
		...
		if (deprecate) {
			global.moment = function() {
				if (!warned && console && console.warn) {
					warned = true;
					console
							.warn("Accessing Moment through the global scope is "
